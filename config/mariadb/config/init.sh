...
EOF
	...
	if [ "$MYSQL_DATABASE" != "" ]; then
		...
		if [ "$MYSQL_USER" != "" ]; then
			echo "GRANT ALL ON \`$MYSQL_DATABASE\`.* to '$MYSQL_USER'@'10.5.0.2' IDENTIFIED BY '$MYSQL_PASSWORD';" >>$tfile
		fi
	fi
	...
	if [ "$MYSQL_DATABASE" != "" ] && [ "$(ls -A /docker-entrypoint-initdb.d 2>/dev/null)" ]; then
		...
		for f in /docker-entrypoint-initdb.d/*; do
			...
			*.sql)
				echo "  $0: running $f"
				eval "${MYSQL_CLIENT} ${MYSQL_DATABASE} < $f"
				echo
				;;
			*.sql.gz)
				echo "  $0: running $f"
				gunzip -c "$f" | eval "${MYSQL_CLIENT} ${MYSQL_DATABASE}"
				echo
				;;
			esac
		done
